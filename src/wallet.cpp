...
bool CWallet::Unlock(const SecureString& strWalletPassphrase)
{
...

    {
		...
		ProcessLockedAnonOutputs(); //Process Locked Anon Outputs when unlocked, CAT E N A R I U S - v3.1
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                ...
                if(coin_type == ONLY_DENOMINATED) {
                    ...
                    found = !IsDenominatedAmount(pcoin->vout[i].nValue);
                    if(found && coin_type == ONLY_NONDENOMINATED_NOTMN) found = (pcoin->vout[i].nValue != GetMNCollateral()*COIN); // do not use MN funds 5,000 CAT
                } else {
                    found = true;
                }
                if(!found) continue;
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            while (true)
            {
                ...
                // vouts to the payees with UTXO splitter - CAT E N A R I U S
        ...
        if(!fortunastakePayments.GetBlockPayee(pindexPrev->nHeight+1, payee)){
            int winningNode = GetFortunastakeByRank(1);
                if(winningNode >= 0){
                    ...
                    else burnAddress = "CATXXXXXXXXXXXXXXXXXXXXXXXXXZeeDTw";
...
void CWallet::FixSpentCoins(int& nMismatchFound, int64_t& nBalanceInQuestion, bool fCheckOnly)
{
    ...
    BOOST_FOREACH(CWalletTx* pcoin, vCoins)
    {
        ...
        for (unsigned int n=0; n < pcoin->vout.size(); n++)
        {
            ...
            {
                printf("FixSpentCoins found lost coin %s CAT %s[%d], %s\n",
            ...
            {
                printf("FixSpentCoins found spent coin %s CAT %s[%d], %s\n",
                    FormatMoney(pcoin->vout[n].nValue).c_str(), pcoin->GetHash().ToString().c_str(), n, fCheckOnly? "repair not attempted" : "repairing");
                nMismatchFound++;
                nBalanceInQuestion += pcoin->vout[n].nValue;
                if (!fCheckOnly)
...
bool CWallet::SendAnonToAnon(CStealthAddress& sxAddress, int64_t nValue, int nRingSize, std::string& sNarr, CWalletTx& wtxNew, std::string& sError, bool fAskFee)
{
    ...
    {
        sError = "Insufficient Anonymous CAT funds";
        return false;
    };
